...
<template>
    <form @submit.prevent="submitLead" class="p-6 border border-slate-900 text-left flex flex-col gap-4 rounded-b-lg">
        <h2 class="text-2xl font-semibold text-left text-gray-900">Regístrate y recibe más información</h2>
        ...
        <div class="flex flex-col gap-2">
            <div class="text-left">
                <label for="name" class="block text-gray-500 text-sm font-bold mb-2 text-left">Nombre</label>
                <input id="name" v-model="formData.name" type="text" :class="{ 'border-red-500': errors.name }" class="shadow appearance-none border w-full py-2 px-3 text-gray-700 leading-tight text-left
                           bg-white autofill:bg-white rounded
                           focus:outline-none focus:ring-1 focus:ring-gray-700 focus:border-transparent"
                    @input="validateField('name')" />
            </div>

            <div class="text-left">
                <label for="email" class="block text-gray-500 text-sm font-bold mb-2 text-left">Correo
                    electrónico</label>
            ...
            <div class="flex gap-2 w-full">
                <div class="text-left flex flex-col w-full">
                    <label for="phone" class=" text-gray-500 text-sm font-bold mb-2 text-left">País</label>
                    <div class="flex flex-row gap-2 relative w-full" ref="countryDropdownRef"
                        ...
                        <div class="relative w-full">
                            <input v-model="countryInputDisplay" :class="{ 'border-red-500': errors.phoneCountryCode }"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight text-left bg-white focus:outline-none focus:ring-1 focus:ring-gray-700 focus:border-transparent"
                                placeholder="País" />
                            ...
                            <div v-if="showCountryDropdown || countrySearchQuery.length > 0"
                                ...
                                <ul>
                                    <p v-for="country in filteredCountries" :key="country.code"
                                        ...
                                        class="cursor-pointer px-3 pt-0.5 hover:bg-gray-100 text-sm rounded">
                                        {{ country.name }}
                                    </p>
                                </ul>
                            </div>
                ...
                <div class="text-left w-full flex flex-col">
                    <label for="phone" class="block text-gray-500 text-sm font-bold mb-2 text-left">Número de
                        teléfono</label>
        ...
        <button type="submit" :disabled="!isFormValid"
            ...
            class="text-white font-bold py-2 px-4 focus:outline-none focus:shadow-outline w-full text-left rounded">
            Enviar
        </button>
    </form>
</template>