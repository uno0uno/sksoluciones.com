...
<template>
  <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-start py-12 px-4">
    <ClientOnly>
      <div class="w-full flex border rounded border-slate-900 overflow-hidden h-full">
    ...
    <div class="flex flex-col h-full w-full">
      <div class="flex flex-col gap-2 items-start px-6 py-2 border-x border-t border-slate-900 rounded-t-lg">
        <h1 class="text-2xl font-bold text-left leading-tight text-gray-900">{{ title }}</h1>
        
        <div class="text-md text-gray-600 text-left font-normal flex flex-col gap-2">
          ...
          <button v-if="description.length > 150" @click="showFullDescription = !showFullDescription" class="text-slate-600 font-semibold hover:text-slate-900 focus:outline-none flex items-center gap-2">
